...
$archivo = $doc->output();
file_put_contents($pdf, $archivo);
$usuarios = $conn->prepare("SELECT * FROM usuarios_scs0_oxerev WHERE rol_scs0_rev = 2");
$usuarios->execute();
$dato = $usuarios->fetchAll(PDO::FETCH_OBJ);
foreach($dato as $data){
    if($data->correo_scs0_rev != ''){
        ...
        try{
            ...
            echo "<div class='alert alert-danger' role='alert'>Por favor intente nuevamente<br>Hubo un error con el envío de correo masivo {ev2[100]}: " . $mail->ErrorInfo . "</div>";
        }
    }
    $mail->clearAddresses();
}
